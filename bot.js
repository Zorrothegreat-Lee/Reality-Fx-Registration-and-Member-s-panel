...
(function () {
  ...
  function currentStudent() {
    try {
      const id = localStorage.getItem('rfx_member_session');
      if (!id) return null;
  ...
  function ask(q) {
    ...
    if (has('sign in') || has('signin') || has('log in') || has('login') || has('password') || has('forgot') || has('code')) {
      if (!enr) return { text: 'Students sign in to the member panel with the <b>email on your enrollment</b> plus your <b>Student Code</b> (the 6-character code you got at registration — your ID is RFX-xxxxx, your code is different).<br>Forgot your code? In production, Firebase handles your password — use "forgot password" there. In this demo, contact the reception team via the Mailbox.', chips: ['My balance', 'What is my status?'] };
    ...
    if (has('human') || has('person') || has('agent') || has('talk') || has('speak')) {
      return { text: 'Totally fair. A real team member is usually seconds away — the reception is covered 24/7. If no one is on duty in this demo, the Mailbox simulates every message the team would send.', chips: ['My balance', 'What is my status?'] };
    }
