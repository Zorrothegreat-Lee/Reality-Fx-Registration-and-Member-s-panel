...
(function () {
  ...
  function ask(q) {
    ...
    if (has('sign in') || has('signin') || has('log in') || has('login') || has('password') || has('forgot') || has('code')) {
      if (!enr) return { text: 'Students sign in to the member panel with the <b>email on your enrollment</b> and your <b>password</b>.<br>Forgot it? Click <b>Forgot password?</b> on the sign-in screen — we email a secure, one-time reset link to your registered address, valid for 15 minutes. I can explain the recovery process, but I can never see or reset your password: the Academy stores only a secure hash, and no staff member will ever ask you for it. No password set yet? Your <b>Student Code / Student ID</b> still signs you in — then set a password from your panel.', chips: ['My balance', 'What is my status?'] };
      return { text: 'You\'re signed in already — what you\'re looking for is on your member panel: your <b>Student ID</b> ' + enr.studentId + ' is under Identity. To change your password, sign out and use <b>Forgot password?</b> on the sign-in screen — the reset link is emailed to your registered address. I never see passwords, and neither do our staff.', chips: ['My balance', 'What is my status?'] };
    }
