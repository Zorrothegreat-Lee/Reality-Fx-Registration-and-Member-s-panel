...
2. **Registration** — the student receives the invoice email, then a registration email with a
   **secure, unique, single-use link** (7-day expiry). The link leads to the branded portal:
   personal details (first name **and** surname — both required, both appear on the
   certificate) → email verification (6-digit code) → CAPTCHA → identity info (phone,
   ...
   - **Surname is required** — a single first name is ambiguous for certificates, SRM
     matching and refund-identity checks, so the portal refuses to continue without it
     (older saves that only have a combined full name are still accepted and split).
   - **Live email path**: the demo has no SMTP server, so the verification step offers an
     "Open your email app" button that opens the student's OWN email client with the code
     pre-filled and addressed to themselves — a genuinely live delivery, their inbox, their
     provider. The simulated Mailbox still carries a copy. Production swaps the whole rail
     for Resend/SendGrid (FOR-LEE §7).
   - **Selfie capture**: **three ways** — "Take on my phone" (opens the **front camera**
     via `capture="user"`), "Use my webcam" (a live **laptop/desktop webcam** preview
     with a capture button via `getUserMedia`, frame downscaled to the same ≤480px
     JPEG as every other selfie), or "Choose from gallery" (an existing photo). The
     dropzone itself defaults to the phone camera. If the webcam is denied or absent,
     the student is told why and offered the other two paths — never stuck. The camera
     stream is released on every close path (✕, backdrop, Cancel, capture), and the
     capture button reports if the camera is still warming up instead of silently
     doing nothing.
...
## The review SLA — an honest clock, not a vague promise

"This usually takes a short time" told a student nothing. Now the under-review screen keeps
an **honest promise**: `security.reviewSlaMinutes` (default **120** — tune it to real
staffing) is what a submitted registration should normally take to be decided.

- **Within the SLA:** the screen shows a live "Submitted 12 min ago · registrations are
  typically decided within ~2 hours — you'll hear by {date+time}" box with a calm
  "most are approved much sooner, no need to keep checking".
- **Past the SLA:** the box flips to the honest version — "3h past our usual window · your
  registration is still safely in our review queue, nothing is wrong and nothing was
  denied · occasionally a registration needs a closer look · if it's urgent, email
  realityfx20@gmail.com with your reference and we'll fast-track it."

The student is never left guessing, and a denial never comes out of nowhere — the same
philosophy as integrity flags: explain, don't black-box. The SLA clock refreshes every 30s
on the submitted screen. (Tune `reviewSlaMinutes` in Staff Console → Security & data
hygiene, or directly in `js/db.js` → `security`.)

## The Trust Bar — how the student carries themselves, drawn in gold

Every approved student's **Trust Bar** starts at **100%** the day their identity is minted.
It is the direct representation of conduct — and the whole Academy reads a student by it:

- **It never sways easily.** Penalties are measured and deliberate (minor −5 / warning −10 /
  serious −20); recovery is earned slowly (+3/+5/+10 for genuine good conduct) and clamped
  at 100. Every move is ledgered against the identity with a reason, a timestamp and who
  applied it — the bar is a record, and the moderator's call is the verdict (flags stay
  review triggers, never auto-judgments).
- **The tiers speak plainly (the founder's model):** 80–100% *Excellent standing* → 50–79%
  *Stable standing* → 30–49% *Caution — be careful* (the student is told "keep it above
  30% or you enter the danger zone") → below 30% *Danger zone* (inside it: ≤25% *Timed
  out* → ≤10% *Extended timeout*) → **0% Restricted** (account fully restricted, pending
  moderator review — the OS stops access). Each tier-crossing fires a notification in the
  student's feed, so nobody wakes up surprised by a sanction.
- **The buddy rule:** a referred student's serious violation costs the referrer points too
  (`referrerPenalty`, default −10) — you vouch for who you bring into the family. That one
  rule quietly forces students to only share their referral code with people they genuinely
  trust, which keeps network quality high. Staff can see the ripple fire via **Simulate OS
  violation** on the SRM profile.
- **On the member panel** the bar is drawn as a **gold percentage RING** — the same visual
  language as the RFX OS course-completion ring, so the two systems speak one design dialect.
  It shows the live score in the centre (e.g. "100% · STANDING") over a dark disc: a clean
  gold arc that draws itself on load and drains as trust falls — **no glow, no diamond, no
  shadows**, deliberately identical to the OS course-completion ring — and tier colours that
  shift from gold → amber → orange → red as the score drops. A restricted (0%) score shows a
  red "restricted" label inside the ring. **Clicking it opens the standing dashboard** —
  every good & bad action that moved the bar, the thresholds explained, and the buddy rule
  stated. A restricted student sees a clear banner.
- **Staff** see the same bar on the SRM profile with **Apply penalty / Credit conduct**
  controls (severity + reason recorded) and the full Trust Bar history.
- **Why it's beautiful on purpose:** a bar students are *obsessed* with keeping full is a
  self-enforcing policy. Same psychology as the demo life bar — see below.

**The demo life bar.** The 24-hour demo countdown carries a **gold life bar** (segmented
gold ticks, clean — no glow, matching the ring's OS style; hover tip "Tour life remaining")
that drains as the tour runs out. Before approval it stays full (no clock yet); the moment the student is
approved it starts draining with the countdown, and at zero it goes dark. One shared clock
(`demoLifeLeft`) feeds both the registration page chip and the member panel — they can never
disagree. The marketing play: students wish the bar never emptied, and when their own real
pass arrives they treat the Trust Bar the same way.

**Landscape member panel.** The member dashboard runs a wide multi-column layout (identity +
Trust ring up top, then courses, vital details, wallet, referral, spend & merch cards in a
responsive grid) instead of the narrow registration shell — every card gets room to breathe
while keeping the same cards, actions and data.

## Student notifications — moments that deserve a toast

A small per-enrollment feed (`enr.notifications`) makes sure students hear about things
that happened while they weren't looking:

- **Print trust granted** — "You have earned Reality FX's trust — you can now print your
  course material."
- **Awards & giveaways** — "You won R1,000 — landed in your RFX account, prize money
  never expires."
- **Referral commission paid** — "Your friend is fully locked in — R526.50 (15% tier)
  landed in your RFX account. Well done for growing the RFX family."
- **Buddy linked** — the moment a referred student is approved, the referrer hears it:
  "Your friend has been approved at Reality FX — their commission vests after the 30-day
  refund window, then lands in your RFX account."

The member panel renders the feed as **"Your moments"** — a card at the top with a gold
**NEW** badge on unread items and a **Mark all read** action. The freshest unread-and-
untoasted item is toasted the moment the panel opens (`markToasted` stamps `toastedAt`;
`toastNewNotifications` fires it), so a special moment is celebrated even if the student is
away when it happens — **and the NEW badge stays until the student acknowledges it**, because
toasting and reading are tracked separately (`toastedAt` vs `read`). Every notification is
written in `db.js` via `notifyStudent`/`notifyEmail`, so future events can join the feed by
...
**The demo countdown.** Demo-pass students see a **live countdown clock** so they never have to
wonder when their tour ends — a gold chip in the registration page header that ticks every
second (hover: *"Time left till your demo session is expired"*) and a matching inline clock in
the member panel identity card. The clock runs on **Academy time**: it starts when the student is
**approved** (that's when the learning can begin — the Academy teaches trading, not
registration), so before approval the chip reads the full window ahead ("24h academy pass ·
begins when your registration is approved") and only after approval does it tick down live. It
turns warn-red with "Expired — request a new pass" at zero. Both surfaces share one clock
(`db.demoTimeLeft` / `db.fmtCountdown`), so the registration page and the panel can never
disagree. Students without a demo pass see no clock at all.

**Why a demo student can show APPROVED/ACTIVE with a countdown running (not a bug):** the
pass walks the full pipeline *by design* — a demo student registers, gets approved, receives a
real Student ID, and the clock then ticks down the tour window from that approval moment.
"Approved" is the student's genuine status inside the tour; the countdown is only how long the
free tour lasts. Once approved, the member-panel clock is labelled **"Demo tour"** with a
tooltip making the permanence of the approval explicit — the label stays "Demo session" only
before approval (where the tour is still about registration).
...
## One-click full audit — the student-complaint button

On the Staff Console → Security & data hygiene there is a **Run full audit** button. It
re-proves the whole chain live, in one click, with zero residue (scratch records are removed
and every counter it touches is restored — auditing never changes the data it audits):

1. **The journey** — a scratch student runs the REAL pipeline: webhook → enrollment →
   invoice → registration invite → link open → submit → approve → identity → handoff →
   ACTIVE. Any broken link fails this.
2. **Money reconciles** — the ledger's liability equals wallet balances + queued cash-outs.
3. **Wallet hygiene** — no negative wallets, no orphan credit.
4. **Identity integrity** — ACTIVE students all hold an ID; Student Codes are unique; no
   stuck submissions.
5. **Store health** — the multi-tab revision guard is alive.
6. **Capacity** — honest headroom in students.
7. **Security self-test** — all four guards fired against real code.

Student complains about anything → one click → PASS/FAIL per check with a verdict header.

## The Academy prep guide — the 'what to bring to school' letter

Every student is sent an **Academy preparation guide** the moment they are approved (before
any handoff — no approved student is left wondering what happens next). It covers what to
prepare (device, journal, space), what every dashboard is for (members panel, mailbox, RFX OS
Academy), what the wallet can do, the rules that keep everyone safe, and how to get help —
and it is **year-stamped** (`ACADEMY_YEAR`, currently 2026): when the year turns, staff change
one constant and the same letter serves the next Academy year; system changes are announced
in the same channel.

- The email is branded, sectioned, and lives in the student's Mailbox like every other
  official message.
- A matching **downloadable PDF** (`RFX.pdf.downloadPrepGuide`) lets students save or print
  it — one page per section, same content, from the member panel's "Academy prep guide"
  card (Download PDF / Email me the guide).
- Staff can re-send it from the student's Registration & Approval tab ("Re-send Academy
  prep guide") — idempotent, and each send is recorded as a security event.
...
- **Lee's contract:** the code on a website product is the same code in this catalog.
  ...
  rather than guessed.
- Catalog items carry a `kind`: `course` (enrollable from the panel), `service`
  (mentorship, seat transfer — “Spend your credit”), or `merch` (the shop).

## Profile tier: DEMO → LIVE

Like Exness/XM (demo until you register live), a student is **DEMO** until their
registration is approved and a Student ID exists — then **LIVE**. The gate is already
enforced (no Student Code exists before approval, so a DEMO account cannot get in);
the badge makes it visible and the terms make it official. `DEMO PROFILE` shows on the
registration “under review” screen; the completion screen and the members panel show
`LIVE PROFILE` with a pulsing gold dot. In production, course access keys on the
underlying `approved`/`studentId` fields, never on the label string.

## Enrolling in another course (from the members panel)

An approved member can buy **another course directly from the panel** — no
re-registration, no forms, no new ID. Their details are already established, so it's:
pick a course from the catalog → pay from the RFX wallet → done.

- Only `kind: 'course'` catalog items are enrollable; services and merch refuse with a
  pointer to the right surface.
- Payment rides the same idempotent spend rail (reference `ADD-<CODE>-<email>`) so a
  retry can never double-charge.
- The new enrollment is created **under the same identity** — same Student ID, same
  Student Code (one person, one credential) — pre-verified, instantly approved, and
  handed to the OS in the same tick.
- **Ownership is enforced twice:** the ADD-reference idempotency AND by course name
  (the original purchase may carry a different transaction ID from the PayPal
  webhook), so a panel retry can never double-buy the same course.
- The “Your courses” card lists every course under the identity with its live state.
